...
try:
    ...
    print("Warning: biosppy not installed. Please install it with: pip install biosppy")
    ecg = None
...
def preprocess_ecg_signal(input_signal, sampling_rate=400, target_length=186):
    ...
    print(f"After resampling to 125Hz: {len(resampled_signal)} samples")
    ...
    if len(resampled_signal) > target_length:
        ...
        print(f"Signal truncated to {target_length} samples")
    else:
        ...
        print(f"Signal padded with {zerocount} zeros to reach {target_length} samples")
    
    return [resampled_signal]

# def preprocess_ecg_signal_with_rpeak_detection(input_signal, sampling_rate=400, target_length=186):
#     """
#     Tiền xử lý tín hiệu ECG với R-peak detection (chỉ dùng cho tín hiệu dài > 2000 samples)
#     """
#     if ecg is None:
#         raise ImportError("biosppy library is required. Install with: pip install biosppy")
    
#     # Kiểm tra nếu tín hiệu quá ngắn cho biosppy (cần ít nhất 2000 samples)
#     if len(input_signal) < 2000:
#         print(f"Signal too short ({len(input_signal)} samples) for R-peak detection. Using simple preprocessing.")
#         return preprocess_ecg_signal(input_signal, sampling_rate, target_length)
    
#     # Detect R-peaks trong tín hiệu ECG
#     try:
#         out = ecg.ecg(signal=input_signal, sampling_rate=sampling_rate, show=False)
#         rpeaks = out['rpeaks']
#         print(f"Detected {len(rpeaks)} R-peaks")
#     except Exception as e:
#         print(f"Warning: R-peak detection failed ({str(e)}). Using simple preprocessing.")
#         return preprocess_ecg_signal(input_signal, sampling_rate, target_length)
    
#     if len(rpeaks) < 2:
#         print("Less than 2 R-peaks detected. Using simple preprocessing.")
#         return preprocess_ecg_signal(input_signal, sampling_rate, target_length)
    
#     # Split thành từng heartbeat dựa trên R-peaks
#     beats = np.split(input_signal, rpeaks)
#     processed_beats = []
    
#     for idx in range(1, len(beats) - 1):  # Bỏ beat đầu và cuối
#         beat = beats[idx]
        
#         # Append 40 giá trị đầu của beat tiếp theo (như trong MIT-BIH processing)
#         if idx + 1 < len(beats):
#             next_beat_portion = beats[idx + 1][:min(40, len(beats[idx + 1]))]
#             beat = np.append(beat, next_beat_portion)
        
#         # Normalize về range 0-1
#         if np.ptp(beat) > 0:  # Tránh chia cho 0
#             beat = (beat - beat.min()) / np.ptp(beat)
        
#         # Resample từ sampling_rate xuống 125Hz
#         newsize = int((beat.size * 125 / sampling_rate) + 0.5)
#         beat = signal.resample(beat, newsize)
        
#         # Bỏ qua nếu beat quá dài
#         if beat.size > target_length:
#             continue
        
#         # Pad với zeros đến target_length
#         zerocount = target_length - beat.size
#         beat = np.pad(beat, (0, zerocount), 'constant', constant_values=(0.0, 0.0))
...
if __name__ == "__main__":
    ...
    # Dự đoán cho beat đầu tiên (tương thích với code cũ)
    # Đoạn code này gọi hàm predict_single_beat để dự đoán cho một heartbeat đơn lẻ
    # Hàm này được giữ lại để tương thích với code cũ, chỉ trả về kết quả của beat đầu tiên
    # Kết quả bao gồm:
    # - predicted_class: số lớp dự đoán (0-4)
    # - confidence: độ tin cậy của dự đoán (0-1)
    # - class_name: tên lớp dự đoán (N, S, V, F, Q)
